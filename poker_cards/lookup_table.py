...
class HashTable(object):
    ...
    * 7-5-4-3-2 unsuited (worst hand possible)  => 7462
    ...
    def flushes(self):
        ...
        rank = 1
        for sf in straight_flushes:
            prime_product = cards.Hand.hand_prime_product_rankbits(sf)
        ...
        rank = HashTable.MAX_FULL_HOUSE + 1
        for f in flushes:
            prime_product = cards.Hand.hand_prime_product_rankbits(f)
            self.flush_lookup[prime_product] = rank
            rank += 1
    ...
    def straight_and_highcards(self, straights, highcards):
        ...
        """
        rank = HashTable.MAX_FLUSH + 1

        for s in straights:
            prime_product = cards.Hand.hand_prime_product_rankbits(s)
            self.unsuited_lookup[prime_product] = rank
            rank += 1

        rank = HashTable.MAX_PAIR + 1
        for h in highcards:
            prime_product = cards.Hand.hand_prime_product_rankbits(h)
            self.unsuited_lookup[prime_product] = rank
            rank += 1
    ...
    def multiples(self):
        ...
        """
        backwards_ranks = list(range(len(cards.Card.rank_integers) - 1, -1, -1))

        # 1) Four of a Kind
        rank = HashTable.MAX_STRAIGHT_FLUSH + 1
        ...
        for i in backwards_ranks:
            ...
            kickers.remove(i)
            for k in kickers:
                product = cards.Card.primes[i] ** 4 * cards.Card.primes[k]
                self.unsuited_lookup[product] = rank
                rank += 1

        # 2) Full House
        rank = HashTable.MAX_FOUR_OF_A_KIND + 1
        ...
        for i in backwards_ranks:
            ...
            pairranks.remove(i)
            for pr in pairranks:
                product = cards.Card.primes[i] ** 3 * cards.Card.primes[pr] ** 2
                self.unsuited_lookup[product] = rank
                rank += 1

        # 3) Three of a Kind
        rank = HashTable.MAX_STRAIGHT + 1
        ...
        for r in backwards_ranks:
            ...
            for kickers in gen:
                c1, c2 = kickers
                product = cards.Card.primes[r] ** 3 * cards.Card.primes[c1] * cards.Card.primes[c2]
                self.unsuited_lookup[product] = rank
                rank += 1

        # 4) Two Pair
        rank = HashTable.MAX_THREE_OF_A_KIND + 1
        ...
        for tp in tpgen:
            ...
            for kicker in kickers:
                product = cards.Card.primes[pair1] ** 2 * cards.Card.primes[pair2] ** 2 * cards.Card.primes[kicker]
                self.unsuited_lookup[product] = rank
                rank += 1

        # 5) Pair
        rank = HashTable.MAX_TWO_PAIR + 1
        ...
        for pairrank in backwards_ranks:
            ...
            for kickers in kgen:
                k1, k2, k3 = kickers
                product = cards.Card.primes[pairrank] ** 2 * cards.Card.primes[k1] \
                          * cards.Card.primes[k2] * cards.Card.primes[k3]
                self.unsuited_lookup[product] = rank
                rank += 1
